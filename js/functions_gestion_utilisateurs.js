...
$(document).ready(function () {
    ...
    //Initialisation
    $.getJSON('gestion_utilisateurs_fonctionnalites_list', {
        ajax: 'true'
    }, function (result) {
        var htln = "";
        for (var i = 0; i < result.length; i++) {
            console.log("Section " + i);
            console.log("Code Strcuture " + result[i].id);
            console.log("Designation" + result[i].intitule);
            htln += '<option value=';
            htln += "" + result[i].id;
            htln += '>';
            htln += "" + result[i].intitule;
            htln += '</option>';
        }
    ...
    var grid = $("#data-table-command").bootgrid({
        ...
        grid.find('button.compte-suppr.extern').on("click", function (e) {
            ...
            rows[0] = $(this).data("row-id");
            var idUtilisateur = $($(this).closest('tr')).find('td').eq(1).text();
            window.location.href="gestion_utilisateurs_get_utilisateur?id_utilisateur="+idUtilisateur;
            ...
            $.ajax(
                {
                    type: "post",
                    dataType: 'json',
                    url: "gestion_utilisateurs_utilisateur_create",
                    data: {
                        nom: in_nom,
                        prenom: in_prenom,
                        passwd: in_passw,
                ...
                .done(function (data) {
                    if (JSON.parse(data) == "100") {
                        ...
                        window.location.replace("management_gestion_utilisateurs_utilisateurs");
                    }
                    else if (JSON.parse(data) == "102")
                        swal("Erreur", "l'utilisateur Existe deja verifier votre Id Utilisateur", "error");
                    else if (JSON.parse(data) == "103")
                        swal("Erreur", "Le Champ Repeter Mot de passe ne Correspond pas au Mot de passe ", "error");
                    else if (JSON.parse(data) == "104")
                        swal("Erreur", "Erreur lors de l'ajout d'Utilisateur, Veuillez réessayer ", "error");    
                    ...
                    $.ajax(
                        {
                            type: "POST",
                            dataType: 'json',
                            url: "gestion_utilisateurs_utilisateur_remove",
                            data: {id_utilisateur: idUtilisateur},
                        }
                    ).done(function (data) {
                        if (JSON.parse(data) == "100") {
